...
type Config struct {
	...
	ContextKeys        []string     `json:"context_keys"`         // ContextKeys specifies the keys to be added to log from context.
...
func Init(ctx context.Context, config Config, additionalAttrs ...any) (context.Context, *slog.Logger, error) {
	...
	case OutputOtel:
		defaultHandler, ctx, err = otelMode(ctx, defaultConfig)
		if err != nil {
			return ctx, nil, err
		}
	case OutputFanout:
		...
		otelModeHandler, ctx, err := otelMode(ctx, defaultConfig)
		if err != nil {
			return ctx, nil, err
		}
